...
The OP_RETURNs are due by Thu 12 Mar 23:59 (CET) or block #4863831.  Given the normal block arrival time, that should be at some point around the midnight.  (If blocks come out super quickly, we'll extend the block deadline as this class is still based on unix time rather than block height.)
...
You need to look for unspent outputs which are sent to the same address.  The private key which led to this script is the double-sha256 of "\<year of Bitcoin mainnet launch\>-\<name of the first block in blockchain\>-\<name of upgrade containing Schnorr signatures\>". (Note: the dashes need to remain in the password, but the angle brackets no, i.e., the password could be 2026-kristian-kostal)

You're going to find and claim an unspent output in this transaction. USE JUST ONE TO YOUR ADDRESS. Please be nice and leave the rest of them for your other classmates!
...
You will need to modify 
	...
	prevAddressString (the address of the "secret..." pubkey)
...
<mark>You will get 4 points if you achieve to do everything AS in this tutorial, i.e. sending exactly 1 transaction from our address to your own private and aftewrwards creating second transaction from your own private address to your own private address but one output is OP_RETURN with all the information asked for in this exercise.
You will get 2 points if you succeed to get an unspent UTXO, but do not succeed in sending OP_RETURN or you send OP_RETURN straight from the first address.</mark>
